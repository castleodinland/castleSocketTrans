...
version_info = "imei_cal_match_it version v1.1105Beta"
    ...
    for i in range(0,len(file)):
        ...
        if isdir(fullfilename[i]):#only save path
            ...
            if matchit:
                ...
                #check DAUL DATA
                dmatch, dmismatch, derrors = filecmp.cmpfiles(str(fullfilename[i]) + gNvCalPath, "REFCALIBRAT\\DUAL", DUALCalFileList, shallow=False)
                qmatch, qmismatch, qerrors = filecmp.cmpfiles(str(fullfilename[i]) + gNvCalPath, "REFCALIBRAT\\QUAD", QUADCalFileList, shallow=False)
...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.lb_title = Label(root,text = version_info)
