...
class Application(Frame):
            ...
            if isdir(fullfilename[i]):#only save path
                ...
                if matchit:
                    ...
                    for tpath in self.ref_path_list:
                        ...
                        #check DAUL DATA
                        dmatch, dmismatch, derrors = filecmp.cmpfiles(join(str(fullfilename[i]), gNvCalPath), join(self.config_ini_data['ref_calibrat_dir'], tpath, "DUAL"), self.DUALCalFileList, shallow=False)
                        ...
                        if(len(qmatch) == 4):
                            if fullfilename[i][-32:] not in self.lQUADDefaultCalByChipID:
    ...
    def check_ref_cal_files(self):
        ...
        for filename in self.DUALCalFileList:
            refCalFiles.append(join('DUAL', filename))
        for filename in self.QUADCalFileList:
            refCalFiles.append(join('QUAD', filename))
    ...
    def read_from_config_ini(self):
        isexist = os.path.isfile('config.ini') 
        if(isexist):
            print "config.ini existed."
        else:
            iniConfigHd = open("config.ini",'w')
            print "create config.ini by default.\n"
